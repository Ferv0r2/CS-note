...
- 중앙 통신 회선 하나에 여러 개의 노드가 연결되어공유하는 네트워크 구성
- 근거리 통신망(LAN)에서 사용함
- 설치 비용이 적고 신뢰성이 우수함
...
- 충돌이 발생되는 가능성이 적으며 노드의 고장 발견을 쉽게 찾을 수 있음
- 네트워크 구성 변경이 어려움
...
- 네트워크의 구조라고도 일컫는 토폴로지가 중요한 이유는 병목 현상을 찾을 때 중요한 기준이기 때문
- 네트워크 토폴리지를 확인하여 서버와 서버 간 그리고 게이트웨이로 이어지는 회선을 추가하여 해결하거나 지연 시간을 짧게 만들기 위해 대역폭을 크게 설정하는 등으로 병목 현상을 해결할 수 있음
...
- 해당 노드의 패킷 수신 상태와 도달하기까지 시간 등을 확인할 수 있음
- 네트워크의 연결여부를 확인할 수 있음
- TCP/IP 프로토콜 중에서 ICMP 프로토콜을 통해 동작하며 ICMP를 지원하지 않거나 traceroute를 차단하는 대상의 경우 ping 테스팅은 불가능함
...
- 유선 LAN 프로토콜, 유선으로 LAN을 구축할 떄 쓰이는 프로토콜
- ex. HTTP, 서로 약속된 인터페이슨느 HTTP라는 프로토콜을 통해 노드들은 웹 서비스를 기반으로 데이터를 주고 받음
